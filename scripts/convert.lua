...
local nvim_lspconfig = vim.env.NVIM_LSPCONFIG or (vim.fn.fnamemodify(repo_root, ":h") .. "/nvim-lspconfig")
...
local function convert(name, cfg)
	...
	elseif type(cfg.cmd) == "function" then
		warns[#warns + 1] =
			"cmd was a Lua function (dynamic command resolution); set `cmd` manually or kakehashi will skip this server"
	else
